...
describe('composant <MentionsLegales />', ()=>{
  ...
  it('The div whitch id is "mentions-legales" contains 2 divs whitch id are "titre-ml" and "my-modal"', ()=>{
    const divMentionsLegales = mentionsLegales.find('#mentions-legales');
    const divTitreMl = divMentionsLegales.find('#titre-ml');
    const divMyModal = divMentionsLegales.find('#my-modal');
  ...
  it('The div whitch id is "my-modal" constains a div whitch className is "modal-content"', ()=>{
    const divMyModal = mentionsLegales.find('#my-modal');
    const divModalContent = divMyModal.find('.modal-content');
    expect(divModalContent).to.have.length(1);
  })

  it('The div whitch className is "modal-content" contains a div whitch className is "close" and a div whitch id is "text-ml"', ()=>{
    const divModalContent = mentionsLegales.find('.modal-content');
